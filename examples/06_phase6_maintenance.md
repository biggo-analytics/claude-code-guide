...
Phase นี้ **ไม่ได้เป็นเส้นตรง** แต่เป็น 7 สถานการณ์อิสระที่เกิดขึ้นได้ในลำดับใดก็ได้:
...
| 6 | Incident Response — Production Down | 👤 คุณแบ็ค, 👤 คุณเอส |
| 7 | Data Migration — เปลี่ยน Schema | 👤 คุณเอส, 👤 คุณแบ็ค, 👤 คุณดีบี (DBA) |
...
## Scenario 6: Incident Response — Production Down

### สถานการณ์

เช้าวันจันทร์ 08:30 น. — ทีม support ได้รับแจ้งจากลูกค้าหลายรายว่าเว็บไซต์ไม่สามารถสั่งซื้อสินค้าได้ ระบบตอบกลับ HTTP 500 ทุกครั้งที่กดยืนยันออเดอร์

### ขั้นตอน Incident Response

#### Step 1: Triage — ระบุขอบเขตปัญหา

👤 คุณแบ็ค เปิด Claude Code ทันที:

```
ดู error logs ล่าสุด 50 บรรทัดจาก docker logs shopfast-api
แล้วสรุปว่า:
1. Error เกิดที่ไหน (handler/service/repository)
2. Error เริ่มเมื่อไร
3. กระทบ endpoint ไหนบ้าง
```

Claude Code วิเคราะห์ logs แล้วพบว่า error เกิดที่ `order_service.go` ตั้งแต่ 07:45 น. — connection pool ของ database เต็มเพราะมี long-running transaction ที่ไม่ถูก close

#### Step 2: Hotfix — แก้ปัญหาเร่งด่วน

```
อ่าน internal/service/order_service.go ฟังก์ชัน CreateOrder
หาจุดที่ database transaction อาจไม่ถูก close ในทุก path
เช่น error path ที่ return ก่อน commit/rollback
แก้ให้ใช้ defer tx.Rollback() pattern
```

```go
// ❌ ก่อนแก้ — error path ไม่ rollback
func (s *OrderService) CreateOrder(req CreateOrderRequest) error {
    tx := s.db.Begin()

    if err := s.validateStock(tx, req.Items); err != nil {
        return err  // ← transaction ค้าง!
    }
...
// ✅ หลังแก้ — defer rollback ทุก path
func (s *OrderService) CreateOrder(req CreateOrderRequest) error {
    tx := s.db.Begin()
    defer func() {
        if r := recover(); r != nil {
...
```
เขียน test ที่จำลองสถานการณ์ error ใน validateStock
ตรวจว่า transaction ถูก rollback ไม่ค้าง
แล้วรัน go test ./internal/service/... -v
```

#### Step 4: Post-mortem

```
เขียน post-mortem report สำหรับ incident วันนี้:
- Timeline (เกิดเมื่อไร, ตรวจพบเมื่อไร, แก้เมื่อไร)
- Root cause
- Impact (กี่ออเดอร์ที่ fail)
- Fix ที่ทำ
- Action items เพื่อป้องกัน
```

✅ **Checkpoint:** ระบบกลับมาทำงานปกติภายใน 45 นาที มี regression test ป้องกัน และ post-mortem สำหรับทีม

⚠️ **Warning:** Hotfix ต้องผ่าน test ก่อน deploy เสมอ — ถึงจะเร่งแค่ไหน อย่าข้าม test

💡 **Tip:** สร้าง incident response runbook ไว้ใน CLAUDE.md — เมื่อเกิดเหตุ ให้ Claude อ่าน runbook แล้วทำตามขั้นตอน

---

## Scenario 7: Data Migration — เปลี่ยน Schema โดยไม่ Downtime

### สถานการณ์

Business team ต้องการเพิ่ม "หมวดหมู่ย่อย" (subcategory) สำหรับสินค้า ปัจจุบัน product มีแค่ category_id เดียว ต้องรองรับ subcategory แบบ hierarchical โดยไม่ downtime

### วางแผน migration strategy

👤 คุณเอส ใช้ Claude Code วางแผน:

```
ปัจจุบัน products มี category_id (FK → categories)
ต้องการเพิ่ม subcategory แบบ parent-child relationship
วางแผน migration strategy ที่:
1. ไม่มี downtime
2. backward compatible กับ code เดิม
3. สามารถ rollback ได้
เขียนเป็น step-by-step plan
```

Claude Code ร่างแผน 4 phases:

### Phase A: เพิ่ม column ใหม่ (ไม่กระทบ code เดิม)

```
สร้าง migration ที่:
1. เพิ่ม parent_id column ใน categories table (nullable)
2. เพิ่ม subcategory_id column ใน products table (nullable)
3. เพิ่ม index สำหรับ parent_id
ต้องไม่ลบหรือเปลี่ยน column เดิม
```

```sql
-- Migration: add_subcategory_support
...
### Phase B: อัพเดท code ให้รองรับทั้งเก่าและใหม่

```
อัพเดท CategoryRepository และ ProductRepository ให้:
1. อ่าน subcategory_id ถ้ามี ไม่มีก็ใช้ category_id เดิม
2. API response รวมทั้ง category และ subcategory
3. Create/Update product รับ subcategory_id เป็น optional
```
...
```
สร้าง script ที่:
1. สร้าง subcategory records จาก mapping file
2. อัพเดท products.subcategory_id ตาม mapping
3. ทำเป็น batch (1000 records/batch) ไม่ lock ทั้ง table
4. มี dry-run mode สำหรับตรวจก่อนรันจริง
```

### Phase D: Cleanup (หลังยืนยันว่าทุกอย่าง OK)

```
หลังจาก subcategory ทำงานถูกต้อง 1 สัปดาห์:
1. สร้าง migration ที่ set subcategory_id เป็น NOT NULL (ถ้าต้องการ)
2. อัพเดท code ให้ require subcategory_id ใน create/update
3. ลบ fallback logic ออก
```

✅ **Checkpoint:** Migration เสร็จสมบูรณ์ ไม่มี downtime ข้อมูลเดิมไม่หาย code เดิมยังทำงานได้ตลอด transition

📌 ดูแนวทาง database design เพิ่มเติมที่ [บทที่ 6](../06_architecture_design.md) หัวข้อ Database Design

💡 **Tip:** Migration ที่ปลอดภัย = "เพิ่มก่อน ลบทีหลัง" — อย่าลบ column/table ใน migration เดียวกับที่เพิ่มของใหม่
